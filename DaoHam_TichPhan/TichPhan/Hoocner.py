...
def GiaTriCuaDaThucTaiC(HeSoDaThuc, c):
    HeSoThuong = ChiaDaThucChoXTruC(HeSoDaThuc, c)
    SoDu = HeSoThuong.pop()
    return SoDu
...
def TinhWx(HeSoW):
    ...
    for i in range(len(HeSoW)):
        HeSoDaThuc = NhanDaThucVoiXTruC(HeSoDaThuc, HeSoW[i])
    return HeSoDaThuc
